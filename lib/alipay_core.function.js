 ...
 * 远程获取数据，GET模式
 * 注意：
 * 1.文件夹中cacert.pem,cakey.pem是SSL证书请保证其路径有效，目前默认路径是：getcwd().'\\cacert.pem'
 * @param url 指定URL完整路径地址
 * @param cacert_url 指定当前工作目录绝对路径
 * @param cakey_url 指定当前工作目录绝对路径
 * return 远程输出的数据
 */
exports.getHttpsResponseGET = function(url,cacert_url,cakey_url, callback){
    var parsed_url = require('url').parse(url);
...
exports.getHttpResponseGET = function(url,callback) {
    var parsed_url = require('url').parse(url);
